...
!
!-----------------------------------------------------------------------
SUBROUTINE crpa_prepare_q (iq, do_iq, setup_pw)
  !-----------------------------------------------------------------------
  !
  !  This routine prepares a few variables that are needed to control
  !  the CRPA run after the q point has been decided, but before
  ...
  USE qpoint,          ONLY : xq, x_q, comp_iq
  USE control_lr,      ONLY : lgamma
  USE noncollin_module,     ONLY : noncolin, npol, domag
  USE crpacom,         ONLY : recalc_sym
  !
  IMPLICIT NONE
  !
