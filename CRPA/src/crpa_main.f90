...
PROGRAM crpa_main
  !-----------------------------------------------------------------------
  !
  ! This is the main driver of the CRPA code.
  !
  USE kinds,             ONLY : DP
  ...
  ! USE ldaU_hp,           ONLY : perturbed_atom, start_q, last_q, &
...
!      !
     CALL crpa_generate_grids()
     !
     CALL crpa_setup_wannier()
!      !
...
!
     ...
     DO iq = 1, nqs
        ...
        !
        CALL crpa_prepare_q(iq, do_iq, setup_pw)
        !
        ...
        CALL crpa_load_q()
        !
        ! CALL crpa_skip_nscf_by_rotate()
        !
        CALL crpa_prepare_perturbations_q(iq)
        !
...
SUBROUTINE print_preamble()
  ...
  !
  WRITE( stdout, '(/,5X,"=---------------------------------------------------------------------------=")')
  WRITE( stdout, '(/,5X,"   Constrained random phase approximation using the CRPA code based on DFPT  ")')
  WRITE( stdout, '(/,5X,"      Please cite the following papers when using this program:              ")')
  WRITE( stdout, '(/,5X,"         - CRPA code : To be added                                           ")')
  WRITE( stdout, '(/,5X,"=---------------------------------------------------------------------------=")')
  !
  RETURN
  !
