...
!
!----------------------------------------------------------------------
subroutine crpa_dvpsi_pert(dv, ik, nrec)
  !----------------------------------------------------------------------
  !
  ...
  USE units_lr,             ONLY : iuatswfc, iudvwfc, lrdvwfc
  USE control_lr,           ONLY : lgamma
  USE ldaU,                 ONLY : Hubbard_lmax, Hubbard_l, offsetU, nwfcU
  USE crpacom,              ONLY : nqsh
  ...
  REAL(DP), INTENT(IN) :: dv(dfftp%nnr)
  ...
  !
  CALL start_clock ('crpa_dvpsi_pert')
  !
  ! JML TODO: IMPLEMENT
  !
...
!   ENDDO
!   IF (counter /= 1) CALL errore( 'crpa_dvpsi_pert', "One perturbed atom must be specified", 1)
  !
  dvpsi(:, :) = (0.0d0, 0.0d0)
  dvpsi(:, :) = 1.d0 ! JML TODO DEBUG
  !
...
!   proj(:,:) = (0.0d0, 0.0d0)
  ...
  npwq= ngk(ikq)
  !
  ! CALL apply_dpot_bands(ik, nbnd, dv, evc, dvpsi)
!   !
...
!   DEALLOCATE (proj)
!   !
  CALL stop_clock ('crpa_dvpsi_pert')
  !
  RETURN
  !
end subroutine crpa_dvpsi_pert
