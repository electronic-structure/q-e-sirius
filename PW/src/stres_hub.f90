...
SUBROUTINE stres_hub( sigmah )
   ...
   !
   IF (U_projection /= "atomic") CALL errore( "stres_hub", &
                   " stress for this U_projection_type not implemented", 1 )
   IF (lda_plus_u_kind == 1) CALL errore( "stres_hub", &
...
SUBROUTINE dprojdepsilon_k( spsi, ik, ipol, jpol, nb_s, nb_e, mykey, dproj )
   ...
   COMPLEX(DP), ALLOCATABLE :: dwfc(:,:), aux(:,:),  dbeta(:,:),   &
                               aux0(:,:), aux1(:,:), betapsi(:,:), &
                               dbetapsi(:,:), wfatbeta(:,:),       &
                               wfatdbeta(:,:)
   ...
   DO ig = 1,npw
      ...
      END DO
   END DO
   IF (ipol == jpol) dwfc(1:npw,:) = dwfc(1:npw,:) - wfcU(1:npw,:)*0.5d0
   !
